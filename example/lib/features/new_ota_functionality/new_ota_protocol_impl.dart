...
class Esp32OtaPackage implements OtaPackage {
  ...
  StreamSubscription? subscription; // declare subscription as an instance variable
  bool firmwareupdate = false;
  ...
  Future<void> updateFirmware(
    ...
    if (firmwareType == 1 && binFilePath != null ) {
      ...
      binaryChunks = await _getFirmwareFromPicker(200);
    ...
    subscription = notifyCharacteristic.onValueReceived.listen((value) async {
      print("recieved value is $value");
      //int intValue = int.parse(value[2].toString(), radix: 16);
      ...
      if(value[0] == 0xF1) // this basically is checking listener stream
        ...
        int nxt = byteData.getUint16(0); // Used getUint16 for a 2-byte integer
        print("--------- nxt -------- ${nxt}");
    ...
    print("this is file size : $fileSize"); // this is where it is sent to esp32
  ...
  Future<void> sendPart(int position, Uint8List data) async {
    ...
    for (int i=0; i<parts; i++){
      print("created $i parts");   //<------------------------------ Print to debug
      ...
      for (int y=0; y<mtu; y++){
        ...
        toSend.setRange(variableusedbelow, variableusedbelow+list2.length, list2);
    ...
    Uint8List update = Uint8List.fromList([0xFC, ((end - start) ~/ 256), ((end - start) %
        256), (position ~/ 256), (position % 256)]);
